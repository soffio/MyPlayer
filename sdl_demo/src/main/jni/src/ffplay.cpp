...
static int audio_decode_frame(VideoState *is) {
    ...
    if (is->swr_ctx) {
        ...
        ALOGD("wanted_nb_samples=%d, out_count=%d",wanted_nb_samples,out_size);
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len) {
    ALOGD("sdl_audio_callback len=%d:",len);
    VideoState *is = (VideoState *) opaque;
    int audio_size, len1;
...
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
    ALOGD("refresh_loop_wait_event");
    double remaining_time = 0.0;
    SDL_PumpEvents();
    int number = 0;
    while (!(number = SDL_PeepEvents(event, 1, SDL_GETEVENT, SDL_FIRSTEVENT,
                           SDL_LASTEVENT))) {
        ALOGD("refresh_loop_wait_event number=%d",number);
        if (!cursor_hidden
        ...
        if (remaining_time > 0.0) {
            ALOGD("refresh_loop_wait_event remaining_time=%lf",remaining_time);
...
int main(int argc, char **argv) {
    ...
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "11111\n");
...
//parse_options(NULL, argc, argv, options, opt_input_file);
    ...
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "44444\n");
    if (display_disable) {
        video_disable = 1;
    }
